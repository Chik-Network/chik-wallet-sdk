...
test("construct mips memos", (t) => {
  ...
  const memo = klvm.alloc(
    new MipsMemo(
      new InnerPuzzleMemo(
        0,
        [
          RestrictionMemo.timelock(klvm, 100n, true),
          RestrictionMemo.enforceDelegatedPuzzleWrappers(
            klvm,
            WrapperMemo.preventVaultSideEffects(klvm, true)
          ),
        ],
        MemoKind.mOfN(
          new MofNMemo(1, [
            new InnerPuzzleMemo(
              ...
              [],
              MemoKind.member(
                MemberMemo.bls(klvm, PublicKey.infinity(), false, true)
              )
            ),
            new InnerPuzzleMemo(
              1000,
