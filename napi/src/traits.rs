...
use chik_wallet_sdk::Memos;
use klvmr::NodePtr;
use napi::bindgen_prelude::*;

use crate::{KlvmAllocator, Program, PublicKey, SecretKey};
...
pub(crate) trait IntoJsContextual<T> {
    fn into_js_contextual(
        self,
        env: Env,
        this: Reference<KlvmAllocator>,
        klvm_allocator: &mut KlvmAllocator,
    ) -> Result<T>;
}
    ...
    U: IntoJs<T>,
{
    fn into_js_contextual(
        self,
        _env: Env,
        _this: Reference<KlvmAllocator>,
        _klvm_allocator: &mut KlvmAllocator,
    ) -> Result<T> {
        self.into_js()
    }
}
...
impl IntoJsContextual<Option<ClassInstance<Program>>> for Option<Memos<NodePtr>> {
    fn into_js_contextual(
        ...
        let ptr = klvm_allocator
            .0
            .alloc(&memos.value)
            .map_err(|error| Error::from_reason(format!("Failed to allocate KLVM: {error}")))?;
...
impl IntoJs<Uint8Array> for chik::protocol::Program {
    fn into_js(self) -> Result<Uint8Array> {
        Ok(Uint8Array::new(self.to_vec()))
    }
}

impl FromJs<Uint8Array> for chik::protocol::Program {
    fn from_js(js_value: Uint8Array) -> Result<Self> {
        Ok(chik::protocol::Program::from(js_value.to_vec()))
    }
}
