use chik::{
    bls,
    klvm_traits::{klvm_quote, FromKlvm, KlvmEncoder, ToKlvm},
    klvm_utils::{self, CurriedProgram, TreeHash},
    protocol::{self, Bytes32},
    puzzles::nft::{self, NFT_METADATA_UPDATER_PUZZLE_HASH},
};
use chik_wallet_sdk::{self as sdk, HashedPtr, Memos, SpendContext};
use klvmr::{
    run_program,
    serde::{node_from_bytes, node_from_bytes_backrefs},
    ChikDialect, NodePtr, MEMPOOL_MODE,
};
use napi::bindgen_prelude::*;
use paste::paste;

use crate::{
    klvm_value::{Allocate, KlvmValue},
...
type Klvm = Reference<KlvmAllocator>;

#[napi]
pub struct KlvmAllocator(pub(crate) SpendContext);
...
impl KlvmAllocator {
    ...
    #[napi(ts_args_type = "")]
    pub fn nil(&mut self, this: This<Klvm>) -> Result<Program> {
        Ok(Program::new(this, NodePtr::NIL))
    }

    #[napi(ts_args_type = "value: Uint8Array")]
    pub fn deserialize(&mut self, this: This<Klvm>, value: Uint8Array) -> Result<Program> {
    ...
    #[napi(ts_args_type = "first: KlvmValue, rest: KlvmValue")]
    pub fn pair(&mut self, this: This<Klvm>, first: KlvmValue, rest: KlvmValue) -> Result<Program> {
    ...
    #[napi(ts_args_type = "value: KlvmValue")]
    pub fn alloc(&mut self, this: This<Klvm>, value: KlvmValue) -> Result<Program> {
        let ptr = value.allocate(&mut self.0.allocator)?;
        Ok(Program::new(this, ptr))
    }

    #[napi(ts_args_type = "value: NftMetadata")]
    pub fn nft_metadata(&mut self, this: This<Klvm>, value: NftMetadata) -> Result<Program> {
        let metadata = nft::NftMetadata::from_js(value)?;

        let ptr = metadata
            .to_klvm(&mut self.0.allocator)
            .map_err(|error| Error::from_reason(error.to_string()))?;
    ...
    pub fn parse_nft_metadata(&mut self, value: &Program) -> Result<NftMetadata> {
        let metadata = nft::NftMetadata::from_klvm(&self.0.allocator, value.ptr)
            .map_err(|error| Error::from_reason(error.to_string()))?;
...
pub fn curry_tree_hash(tree_hash: Uint8Array, args: Vec<Uint8Array>) -> Result<Uint8Array> {
    ...
    let args: Vec<TreeHash> = args
        ...
        .collect::<Result<Vec<_>>>()?;
    klvm_utils::curry_tree_hash(tree_hash.into(), &args)
        .to_bytes()
        .into_js()
}
    ...
    ( $( $condition:ident $( < $( $generic:ty ),* > )? { $hint:literal $function:ident( $( $name:ident: $ty:ty $( => $remap:ty )? ),* ) }, )* ) => {
        ...
        $( paste! {
            ...
            impl KlvmAllocator {
                ...
                pub fn $function( &mut self, this: This<Klvm>, $( $name: $ty ),* ) -> Result<Program> {
                ...
                #[allow(unused)]
                pub fn [< parse_ $function >]( &mut self, env: Env, this: This<Klvm>, program: Reference<Program> ) -> Result<Option<$condition>> {
                    let Some(condition) = sdk::$condition $( ::< $( $generic ),* > )? ::from_klvm(&self.0.allocator, program.ptr).ok() else {
                        return Ok(None);
                    };
