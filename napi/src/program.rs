...
use chik::{
    bls,
    klvm_traits::{FromKlvm, KlvmDecoder},
    klvm_utils::{tree_hash, CurriedProgram},
};
use klvmr::{
...
use crate::{traits::IntoJs, KlvmAllocator, PublicKey, Signature};

#[napi]
pub struct Program {
    pub(crate) ctx: Reference<KlvmAllocator>,
    pub(crate) ptr: NodePtr,
}

impl Program {
    pub(crate) fn new(ctx: Reference<KlvmAllocator>, ptr: NodePtr) -> Self {
        Self { ctx, ptr }
    }
...
impl Program {
    ...
    pub fn to_list(&self, env: Env) -> Result<Vec<ClassInstance<Program>>> {
        Vec::<NodePtr>::from_klvm(self.alloc(), self.ptr)
    ...
    pub fn uncurry(&self, env: Env) -> Result<Option<Curry>> {
        let Ok(value) = CurriedProgram::<NodePtr, NodePtr>::from_klvm(self.alloc(), self.ptr)
        else {
            return Ok(None);
        };
